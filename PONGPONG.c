		...
		while(show_score == 0) {
			...
			while(show_score == 1)
			{
				if(TMR3CN == 0x84)//If the overflow flag is raised
				{
					...
					blank_screen();		//The screen is blanked and
					ball_x += dx;			//the ball is moved the distance
					ball_y += dy;			//and direction that is being
					init_game();			//in the move_ball function
					get_switches(); 
					move_comps();
					draw_ball(ball_x, ball_y);
					refresh_screen();	
...
void timer_0(void) interrupt 1
{
	...
	dx = -dx;
	if(random_count < sizeof(dy_random)-1)//If less than 10 enter loop
	{
		dy = dy_random[random_count];//set change in y to random value
		random_count++;				 //and increment the count
	}
	else if(random_count == sizeof(dy_random)-1)//If equal to 10 enter loop
	{
		dy = dy_random[random_count];//set change in y to random value
		random_count = 0;			 //and reset the count to 0
	}
}
...
void move_comps(void) {
	...
	paddle2top = ADC0H*4;//so we get value out of 64	
		...
		ballx = ball_x+2; //ballx and bally are the center of the ball
		bally = ball_y+2;
	...
	if (ballx < 3){ 
		...
		//-----FULL PADDLE-----
		if (paddle1 == 3){
		dx = -dx; //INVERT DIRECTION AUTOMATICALLY BECAUSE NO MATTER WHERE IT HITS IT WILL BOUNCE BACK
		sound(1); 
		} 
		//-----TOP QUAD-----									
...
void winner(void) {
			...
			init_game();
			display_scores(score1, score2);
			put_char(1,48, 'W'); put_char(1,54, 'I'); put_char(1,60,'N');	put_char(1,66,'N'); //displays WINNER
			put_char(1,72,'E'); put_char(1,78,'R');
			refresh_screen();
			onemillidelay(2000);
...
void speed(char speed)
{
		if(speed==0) {
			...
			TMR3RLH = (-15360)>>8;//120 pixels per second
			TMR3RLL = -15360;
		}
		else if(speed==2) {
